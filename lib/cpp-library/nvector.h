...
inline bool all_values_equal(Arg&& arg) {
    (void)arg;
    return true;
}
template<typename Arg1, typename Arg2, typename... Args>
inline bool all_values_equal(Arg1&& arg1, Arg2&& arg2, Args&&... args) {
...
struct foreach_dim {
    static inline std::size_t end(std::size_t index, std::array<std::size_t, dim>& pos, const std::array<Slice, dim>& dims) {
        std::get<c>(pos) = std::get<c>(dims).begin + std::get<c>(dims).size;
        return foreach_dim<c + 1, dim>::end(index * std::get<c>(dims).size, pos, dims);
    }
    static inline std::size_t begin(std::size_t index, std::array<std::size_t, dim>& pos, const std::array<Slice, dim>& dims) {
        std::get<c>(pos) = 0;
    ...
    static inline void increase(std::array<std::size_t, dim>& pos, const std::array<Slice, dim>& dims, std::size_t by) {
    ...
    static inline void pass_parameters_void(const std::array<std::size_t, dim>& pos, Function&& func, Args&&... args) {
        foreach_dim<c + 1, dim, Args...>::template pass_parameters_void<Ns..., c>(pos, std::forward<Function>(func), std::forward<Args>(args)...);
    }
    template<std::size_t... Ns, typename Function>
    static inline bool pass_parameters(const std::array<std::size_t, dim>& pos, Function&& func, Args&&... args) {
...
template<typename Arg>
inline void pass(Arg&&) {}

template<typename Arg, typename... Args>
inline void pass(Arg&&, Args&&... args) {
    pass(std::forward<Args>(args)...);
}

template<typename Arg>
inline bool none_ended(Arg&& it) {
    return !it.ended();
}

template<typename Arg, typename... Args>
inline bool none_ended(Arg&& it, Args&&... its) {
    return !it.ended() && none_ended(std::forward<Args>(its)...);
}

template<typename Function, typename Additional, typename Arg, typename... Args>
inline bool foreach_iterator(Function&& func, Additional&& temp, Arg&& it, Args&&... its) {
    (void)temp;
...
class View {
  public:
    ...
    class SplitViewHandler {
      ...
      public:
        SplitViewHandler(Iterator it_p, std::array<Slice, inner_dim> dims_p) : it(std::move(it_p)), dims(std::move(dims_p)){};
        inline View<T, inner_dim, Iterator, Tref> operator[](std::size_t index) { return {it + index, dims}; }
        inline const View<T, inner_dim, Iterator, Tref> operator[](std::size_t index) const { return {it + index, dims}; }
    };
  ...
  public:
    ...
    View(Iterator it_p, Args&&... args) : it(std::move(it_p)) {
